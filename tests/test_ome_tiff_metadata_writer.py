...
def test_update_metadata_multifile(tmp_path: Path) -> None:
    ...
    # Position 0 should contain ALL images (OME multi-file spec)
    ...
    with tifffile.TiffFile(str(pos0_file)) as tif:
        ...
        ome_obj = from_xml(ome_xml)
        # Position 0 file should contain metadata for ALL positions
    ...
    # Position 1+ should contain BinaryOnly references (OME multi-file spec)
    ...
    with tifffile.TiffFile(str(pos1_file)) as tif:
        ...
        ome_obj = from_xml(ome_xml)
        # Position 1 file should have BinaryOnly reference to position 0
        assert ome_obj.binary_only is not None
        assert ome_obj.binary_only.metadata_file == f"{base_path.name}_p000.ome.tiff"
        assert len(ome_obj.images) == 0  # No images, only BinaryOnly reference
...
def test_update_metadata_with_plates(tmp_path: Path) -> None:
    ...
    # Verify that position files follow OME multi-file spec
    import tifffile
    from ome_types import from_xml

    base_path = Path(str(output_path).replace(".ome.tiff", ""))

    # Position 0 should contain ALL images and complete plate info
    pos0_file = base_path.with_name(f"{base_path.name}_p000.ome.tiff")
    assert pos0_file.exists()
    ...
    with tifffile.TiffFile(str(pos0_file)) as tif:
        ...
        # Position 0 file should have all images
        assert len(ome_obj.images) == 2
        assert ome_obj.images[0].id == "Image:0"
        assert ome_obj.images[1].id == "Image:1"

        # Should have complete plate information
        assert len(ome_obj.plates) == 1
        plate = ome_obj.plates[0]
        assert len(plate.wells) == 2  # Both wells in position 0

    # Position 1+ should contain BinaryOnly references
    pos1_file = base_path.with_name(f"{base_path.name}_p001.ome.tiff")
    assert pos1_file.exists()
    ...
    with tifffile.TiffFile(str(pos1_file)) as tif:
        ...
        # Position 1 file should have BinaryOnly reference
        assert ome_obj.binary_only is not None
        assert ome_obj.binary_only.metadata_file == f"{base_path.name}_p000.ome.tiff"
        assert len(ome_obj.images) == 0  # No images, only BinaryOnly reference
