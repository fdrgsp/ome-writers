...
class TensorStoreZarrStream(MultiPositionOMEStream):
    ...
    def _create_spec(
        ...
        driver = "memory" if self._in_memory else "file"
        return {
            "driver": "zarr3",
            "kvstore": {"driver": driver, "path": str(self._array_paths[array_key])},
            "schema": {
    ...
    def _create_group(self, path: str, dims: Sequence[Dimension]) -> Path | str:
        ...
        for pos_idx in range(num_positions):
            ...
            if self._in_memory:
                ...
                self._array_paths[array_key] = f"{self._group_path}{array_key}"
            else:
